...
class LLM:
    ...
    def __init__(
        self,
        provider: LLMProvider = LLMProvider.QWEN_REMOTE,
    ...
    def get_llamacpp_llm(self):
        ...
        if not DEST.exists():
            raise FileNotFoundError(
                f"Model file {DEST} does not exist. Please run the correct docker profile (docker compose --profile local-gpu up -d) to download the model."
            )
        ...
        return LlamaCpp(
            model_path=DEST,  # type: ignore[call-arg]
