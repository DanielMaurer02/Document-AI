...
def generate_prompt():
    ...
    prompt = PromptTemplate.from_template(
        "Context information is below. Each piece of context includes file_url information in brackets.\n"
        "---------------------\n{context}\n---------------------\n"
        "If you find the context to be relevant, use it to answer the question.\n"
        "If you found relevant context, always include the file url(s) where the information was found and place that information at the bottom. Include the full url, not just the filename. Don't include the corresponding chunk in the source as it is not relevant.\n"
        "Only give the file urls for information that you got from the context, not for information that you already know. Never state that you didn't find file urls for non context information. You can detect file urls that are not coming from the context by looking at this example: [fileurl: file_url, Chunk: chunk_id].\n"
        "Don't include a file urls for information that you already know, even if it is relevant to the question.\n"
        "Only include relevant information from the context.\n"
        "If you list the file url of a context information, list it only once, even if it was used multiple times and different chunks where used.\n"
